...
interface StagePackageJson {
  ...
  readonly glassCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "GLASS_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("GLASS_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "GLASS_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("GLASS_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("GLASS_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("GLASS_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("GLASS_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("GLASS_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("GLASS_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("GLASS_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("GLASS_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
});
...
function stageMacIcons(stageResourcesDir: string, verbose: boolean) {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const icnsSource = yield* ProductionMacIconIcnsSource;
    const iconPngPath = path.join(stageResourcesDir, "icon.png");
    const iconIcnsPath = path.join(stageResourcesDir, "icon.icns");

    if (yield* fs.exists(icnsSource)) {
      yield* fs.copyFile(icnsSource, iconIcnsPath);
      const tmpRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "glass-icon-raster-",
      });
      const tmpPng = path.join(tmpRoot, "from-icns.png");
      yield* runCommand(
        ChildProcess.make({
          ...commandOutputOptions(verbose),
        })`sips -s format png ${icnsSource} --out ${tmpPng}`,
      );
      yield* runCommand(
        ChildProcess.make({
          ...commandOutputOptions(verbose),
        })`sips -z 512 512 ${tmpPng} --out ${iconPngPath}`,
      );
      return;
    }
    ...
    if (!(yield* fs.exists(iconSource))) {
      return yield* new BuildScriptError({
        message: `Production icon source is missing at ${iconSource} (and no ${BRAND_ASSET_PATHS.productionMacIconIcns})`,
      });
    }

    const tmpRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "glass-icon-build-",
    });
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.glass.app",
    productName,
    artifactName: "Glass-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `glass-desktop-${options.platform}-stage-`,
  });
  ...
  if (!options.skipBuild) {
    yield* Effect.log("[desktop-artifact] Building desktop/web artifacts...");
    yield* runCommand(
      ChildProcess.make({
        cwd: repoRoot,
  ...
  if (!(yield* fs.exists(bundledClientEntry))) {
    return yield* new BuildScriptError({
      message: `Missing web bundle at ${bundledClientEntry}. Run 'pnpm run build:desktop' first.`,
    });
  }
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    glassCommitHash: commitHash,
    private: true,
    description: "Glass desktop build",
    author: "Glass",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "Glass",
      options.signed,
      options.mockUpdates,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: GLASS_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: GLASS_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: GLASS_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: GLASS_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: GLASS_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `pnpm run build:desktop` and use existing dist artifacts (env: GLASS_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: GLASS_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: GLASS_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: GLASS_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: GLASS_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription("Mock update server port (env: GLASS_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Glass."),
