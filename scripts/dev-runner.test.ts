...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit GLASS_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "GLASS_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid GLASS_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults GLASS_HOME to ~/.glass when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GLASS_HOME, resolve(homedir(), ".glass"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GLASS_HOME, resolve("/tmp/custom-glass"));
        assert.equal(env.GLASS_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.GLASS_NO_BROWSER, "1");
        assert.equal(env.GLASS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.GLASS_LOG_WS_EVENTS, "1");
        assert.equal(env.GLASS_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            GLASS_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          glassHome: undefined,
        ...
        assert.equal(env.GLASS_MODE, "web");
        assert.equal(env.GLASS_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GLASS_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom glassHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.GLASS_HOME, resolve("/tmp/my-glass"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GLASS_PORT, undefined);
        assert.equal(env.GLASS_AUTH_TOKEN, undefined);
        assert.equal(env.GLASS_MODE, undefined);
        assert.equal(env.GLASS_NO_BROWSER, undefined);
        assert.equal(env.GLASS_HOST, undefined);
