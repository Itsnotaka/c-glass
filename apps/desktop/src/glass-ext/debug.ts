...
function generateFetchSnippet(url: string, label: string, dataExpr: string = "{}"): string {
...
function createDebugServer(port: number, logPath: string): Promise<http.Server> {
  return new Promise((resolve, reject) => {
...
- Compare expected vs actual behavior`,
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const output = params.tail && params.tail > 0 ? lines.slice(-params.tail) : lines;
    ...
    renderResult(result, _opts, theme) {
      const { Text } = require("@mariozechner/pi-tui");
      const details = result.details as { entries?: number; total?: number } | undefined;
      if (details?.entries) {
        const suffix =
...
Use this before a new debugging session to remove old log entries.`,
  ...
  pi.registerTool({
    ...
    renderResult(result, _opts, theme) {
      const { Text } = require("@mariozechner/pi-tui");
      const details = result.details as { active?: boolean; url?: string } | undefined;
      if (details?.active) {
        return new Text(
  ...
  pi.registerCommand("debug", {
    ...
    handler: async (_args, ctx) => {
      if (!debugModeActive || !server) {
        ctx.ui.notify("Debug mode is not active. Ask the agent to use debug_start.", "info");
      } else {
