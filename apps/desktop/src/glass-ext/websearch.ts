...
function truncateContent(content: string): {
  ...
  return {
    text: content.slice(0, MAX_INLINE_CONTENT) + "\n\n[Content truncated to 30,000 characters.]",
    truncated: true,
  };
}
...
async function searchWithParallelCli(
  ...
  signal?: AbortSignal,
): Promise<{ ok: true; data: SearchResponse } | { ok: false; status?: number; message: string }> {
...
async function extractWithParallelCli(
  ...
  const firstError =
    (response.data.errors ?? []).find((item) => item.url === url) ?? response.data.errors?.[0];
  if (firstError) {
    const message =
    ...
    return {
      ok: false,
      ...(firstError.http_status_code !== undefined ? { status: firstError.http_status_code } : {}),
      message,
    };
  }
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const spinnerStage: SpinnerDetails["stage"] = directUrl ? "fetching" : "searching";
      ...
      try {
        ...
        const parallelSearch = await searchWithParallelCli(query, maxResults, maxChars, signal);
        ...
        const rendered = results
          .map((result, index) => `${index + 1}. ${formatResult(result, maxChars)}`)
          .join("\n\n");
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (details?.mode === "fetch") {
        ...
        const providerLabel = provider.startsWith("parallel-cli") ? "parallel-cli" : provider;
        ...
        const latency =
          typeof details.latencyMs === "number" ? theme.fg("dim", ` (${details.latencyMs}ms)`) : "";
        const header =
        ...
        if (details.fallbackReason) {
          lines.push(
            theme.fg("warning", `Fallback reason: ${shorten(details.fallbackReason, 180)}`),
          );
        }
      ...
      const provider = details?.provider ? theme.fg("dim", ` · ${details.provider}`) : "";
