...
function computeCacheKey(repoUrl: string, branch: string, sparsePaths?: string[]): string {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      branch: Type.Optional(Type.String({ description: "Branch to clone (default: auto-detect)" })),
      path: Type.Optional(
        Type.String({
          description:
    ...
    async execute(toolCallId, params, _signal, onUpdate, _ctx) {
      ...
      switch (action) {
        case "create": {
          ...
          const eagerCacheKey = branch ? computeCacheKey(repoUrl, branch, sparsePaths) : null;
          if (eagerCacheKey) {
          ...
          if (!branch) {
            ...
            const lateCacheKey = computeCacheKey(repoUrl, resolvedBranch, sparsePaths);
          ...
          try {
            ...
            const stderr = err.stderr || err.message || "";
            const { kind, hint } = classifyGitError(stderr);
            throw new Error(`Clone failed (${kind}): ${hint}\nRaw: ${stderr.slice(0, 300)}`);
          }
        }
        ...
        case "destroy": {
          ...
          try {
            ...
            throw new Error(`Failed to destroy: ${e instanceof Error ? e.message : String(e)}`);
          }
        }
        ...
        default:
          throw new Error(`Unknown action '${action}'. Use: create, destroy, list.`);
      }
    },
