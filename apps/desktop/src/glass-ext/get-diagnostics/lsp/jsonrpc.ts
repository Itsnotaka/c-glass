...
export type RequestHandler = (method: string, params: any) => Promise<RequestHandlerResponse>;
...
export function createConnection(proc: ChildProcessWithoutNullStreams): JsonRpcConnection {
  let nextId = 1;
  ...
  function parseMessages() {
    while (true) {
      ...
      const contentLen = parseInt(len, 10);
      const bodyStart = headerEnd + 4;
      if (buffer.length < bodyStart + contentLen) break;

      const body = buffer.subarray(bodyStart, bodyStart + contentLen).toString("utf8");
  ...
  proc.on("error", (error) => {
    if (disposed) return;
    rejectPending(error instanceof Error ? error : new Error(String(error ?? "LSP process error")));
  });
