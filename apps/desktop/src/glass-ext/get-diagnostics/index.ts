...
export default function (pi: ExtensionAPI) {
  ...
  const starting = new Map<string, Promise<LspServer | null>>();
  ...
  function clearCachedServer(key: string, server: LspServer) {
    if (servers.get(key) === server) {
      servers.delete(key);
    }
  }

  async function awaitReadyServer(
    key: string,
  ...
  async function getOrSpawnServer(
    ...
    if (!server) {
      ...
      const start = (async () => {
        let started: LspServer;

        try {
          started = await spawnServer(id, root);
        } catch (error) {
        ...
        started.proc.on("exit", () => clearCachedServer(key, started));
        return await awaitReadyServer(key, id, started);
      })().finally(() => {
        starting.delete(key);
      });
    ...
    return await awaitReadyServer(key, id, server);
  }
