...
function formatUsageStats(
  ...
  if (usage.turns) {
    ...
    parts.push(themeFg("muted", `${usage.turns}`) + themeFg("dim", ` ${label}`));
  }
  if (usage.input) parts.push(themeFg("accent", "↑") + themeFg("muted", formatTokens(usage.input)));
  if (usage.output)
    parts.push(themeFg("accent", "↓") + themeFg("muted", formatTokens(usage.output)));
  if (usage.cost) {
...
function formatStatusLine(
  ...
  if (r.usage.turns) {
    ...
    tokenParts.push(themeFg("muted", `${r.usage.turns}`) + themeFg("dim", ` ${label}`));
  }
  if (r.usage.input)
    tokenParts.push(themeFg("accent", "↑") + themeFg("muted", formatTokens(r.usage.input)));
  if (r.usage.output)
    tokenParts.push(themeFg("accent", "↓") + themeFg("muted", formatTokens(r.usage.output)));
  if (r.usage.cacheRead)
    tokenParts.push(themeFg("dim", "R") + themeFg("muted", formatTokens(r.usage.cacheRead)));
  if (r.usage.cacheWrite)
    tokenParts.push(themeFg("dim", "W") + themeFg("muted", formatTokens(r.usage.cacheWrite)));
  if (r.usage.cost) {
    let costColor: "success" | "warning" | "error" = "success";
  ...
  if (r.usage.contextTokens && r.usage.contextTokens > 0)
    tokenParts.push(themeFg("dim", "ctx:") + themeFg("muted", formatTokens(r.usage.contextTokens)));
...
function formatToolCall(toolName: string, args: Record<string, unknown>, themeFg: ThemeFg): string {
  ...
  switch (toolName) {
    ...
    case "ls": {
      ...
      return (
        themeFg("muted", "→ ") + themeFg("muted", "ls ") + themeFg("accent", shortenPath(rawPath))
      );
    }
    case "find": {
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      if (details.mode === "single" && details.results.length === 1) {
        ...
        if (expanded) {
          ...
          container.addChild(
            new Text(theme.fg("muted", "─── Task (full context passed to subagent) ───"), 0, 0),
          );
          container.addChild(new Markdown(r.task, 0, 0, mdTheme));
          container.addChild(new Spacer(1));
          container.addChild(new Text(theme.fg("muted", "─── Subagent Activity ───"), 0, 0));
          if (displayItems.length === 0 && !finalOutput) {
            container.addChild(
              new Text(theme.fg("muted", isRunning ? "Running…" : "(no output)"), 0, 0),
            );
          } else {
          ...
          const statusLine = formatStatusLine(r, theme.fg.bind(theme), theme.bold.bind(theme));
          if (statusLine) {
            container.addChild(new Spacer(1));
      ...
      if (details.mode === "chain") {
        ...
        if (expanded) {
          ...
          for (const r of details.results) {
            ...
            container.addChild(new Text(theme.fg("muted", "Task (full context):"), 0, 0));
            container.addChild(new Markdown(r.task, 0, 0, mdTheme));
            ...
            const stepStatus = formatStatusLine(r, theme.fg.bind(theme), theme.bold.bind(theme));
      ...
      if (details.mode === "parallel") {
        ...
        if (expanded && !isRunning) {
          ...
          for (const r of details.results) {
            ...
            const taskStatus = formatStatusLine(r, theme.fg.bind(theme), theme.bold.bind(theme));
