...
interface CliInput {
  ...
  readonly glassHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("GLASS_MODE").pipe(
    Config.option,
...
const makeServerRuntimeProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Glass running", {
      ...safeConfig,
...
const glassHomeFlag = Flag.string("home-dir").pipe(
...
export const glassCli = Command.make("glass", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  glassHome: glassHomeFlag,
  devUrl: devUrlFlag,
