...
const RUNTIME_MESSAGE_DEDUPE_WINDOW_MS = 5_000;
...
export function classifyCodexStderrLine(rawLine: string): { message: string } | null {
  ...
  if (match) {
    ...
    const isBenignError = BENIGN_ERROR_LOG_SNIPPETS.some((snippet) => normalized.includes(snippet));
    if (isBenignError) {
      return null;
    }
  }
...
export function shouldSuppressCodexRuntimeMessage(
  last: CodexSessionContext["lastRuntimeMessage"],
  message: string,
  at = Date.now(),
): boolean {
  ...
  return (
    ...
    at - last.at <= RUNTIME_MESSAGE_DEDUPE_WINDOW_MS
  );
}
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  constructor(services?: Context.ServiceMap<never>) {
    super();
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      this.attachProcessListeners(ctx);

      this.emitLifecycleEvent(ctx, "session/connecting", "Starting codex app-server");

      await this.sendRequest(ctx, "initialize", buildCodexInitializeParams());

      this.writeMessage(ctx, { method: "initialized" });
      try {
        const modelListResponse = await this.sendRequest(ctx, "model/list", {});
        console.log("codex model/list response", modelListResponse);
      } catch (error) {
        console.log("codex model/list failed", error);
      }
      try {
        const accountReadResponse = await this.sendRequest(ctx, "account/read", {});
        console.log("codex account/read response", accountReadResponse);
        ctx.account = readCodexAccountSnapshot(accountReadResponse);
        console.log("codex subscription status", {
          type: ctx.account.type,
          planType: ctx.account.planType,
          sparkEnabled: ctx.account.sparkEnabled,
        });
      } catch (error) {
      ...
      if (resumeThreadId) {
        try {
          ...
          threadOpenResponse = await this.sendRequest(ctx, "thread/resume", {
            ...sessionOverrides,
          ...
          threadOpenResponse = await this.sendRequest(ctx, "thread/start", threadStartParams);
        }
      } else {
        threadOpenMethod = "thread/start";
        threadOpenResponse = await this.sendRequest(ctx, "thread/start", threadStartParams);
      ...
      this.emitLifecycleEvent(ctx, "session/ready", `Connected to thread ${providerThreadId}`);
      return { ...ctx.session };
    } catch (error) {
  ...
  private attachProcessListeners(context: CodexSessionContext): void {
    ...
    context.child.stderr.on("data", (chunk: Buffer) => {
      ...
      for (const rawLine of lines) {
        ...
        if (this.shouldSuppressRuntimeMessage(context, classified.message)) {
          continue;
        }
  ...
  private handleServerNotification(
    ...
    const runtimeErrorMessage =
      notification.method === "error"
        ...
        : undefined;
    const suppressRuntimeError =
      notification.method === "error" &&
      runtimeErrorMessage !== undefined &&
      this.shouldSuppressRuntimeMessage(context, runtimeErrorMessage);
  ...
  private shouldSuppressRuntimeMessage(
    context: CodexSessionContext,
    message: string,
    at = Date.now(),
  ): boolean {
    if (shouldSuppressCodexRuntimeMessage(context.lastRuntimeMessage, message, at)) {
      return true;
    }
    context.lastRuntimeMessage = {
