...
import { CliConfig, recordStartupHeartbeat, glassCli, type CliConfigShape } from "./main";
...
const runCli = (
  ...
  env: Record<string, string> = { GLASS_NO_BROWSER: "true" },
) => {
  return Command.runWith(glassCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        GLASS_MODE: "desktop",
        GLASS_PORT: "4999",
        GLASS_HOST: "100.88.10.4",
        GLASS_HOME: "/tmp/t3-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        GLASS_NO_BROWSER: "true",
        GLASS_AUTH_TOKEN: "env-token",
      });
  ...
  it.effect("uses bootstrap envelope values as fallbacks when CLI and env are absent", () =>
    Effect.gen(function* () {
      const fd = yield* openBootstrapFd({
        ...
        port: 4888,
        host: "127.0.0.2",
        glassHome: "/tmp/t3-bootstrap-home",
  ...
  it.effect("applies CLI then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["--port", "4999", "--host", "0.0.0.0", "--auth-token", "cli-token"], {
        ...
        GLASS_HOME: "/tmp/t3-env-home",
        GLASS_NO_BROWSER: "true",
        GLASS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
        GLASS_LOG_WS_EVENTS: "true",
      });
  ...
  it.effect("prefers --mode over GLASS_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over GLASS_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        GLASS_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        GLASS_MODE: "desktop",
        GLASS_LOG_WS_EVENTS: "false",
        GLASS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        GLASS_NO_BROWSER: "true",
      });
