import { ApprovalRequestId, type ChatAttachment, type OrchestrationEvent } from "@glass/contracts";
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        case "thread.message-sent": {
          ...
          const nextContent = event.payload.content ?? previousMessage?.content;
