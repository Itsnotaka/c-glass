...
import type { ModelSelection, ProviderRuntimeEvent, ProviderSession } from "@glass/contracts";
import {
  ApprovalRequestId,
  CommandId,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "glass-reactor-"));
  ...
  it("forwards claude effort options through session start and turn send", async () => {
    const harness = await createHarness({
      threadModelSelection: { provider: "pi", model: "claude-sonnet-4-6" },
    });
    const now = new Date().toISOString();
  ...
  it("forwards claude fast mode options through session start and turn send", async () => {
    const harness = await createHarness({
      threadModelSelection: { provider: "pi", model: "claude-opus-4-6" },
    });
    const now = new Date().toISOString();
