...
it.effect("ProviderServiceLive rejects new sessions for disabled providers", () =>
  Effect.gen(function* () {
    ...
    assert.include(failure.issue, "Provider 'pi' is disabled in Glass settings.");
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const firstCodex = makeFakePiAdapter();
      ...
      const secondCodex = makeFakePiAdapter();
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      ...
      const initial = yield* Effect.gen(function* () {
        const provider = yield* ProviderService;
        return yield* provider.startSession(asThreadId("thread-codex-start"), {
          provider: "pi",
          threadId: asThreadId("thread-codex-start"),
          cwd: "/tmp/project-codex-start",
          runtimeMode: "full-access",
        });
      }).pipe(Effect.provide(firstProviderLayer));
      ...
      if (resumedStartInput && typeof resumedStartInput === "object") {
        ...
        assert.equal(startPayload.provider, "pi");
        assert.equal(startPayload.cwd, "/tmp/project-codex-start");
        assert.deepEqual(startPayload.resumeCursor, initial.resumeCursor);
        assert.equal(startPayload.threadId, initial.threadId);
      }
