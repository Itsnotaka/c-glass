...
import { decodeJsonResult, formatSchemaError } from "@glass/shared/schemaJson";
 ...
 */
export class Server extends ServiceMap.Service<Server, ServerShape>()("glass/wsServer/Server") {}
...
> {
  ...
  httpServer.on("upgrade", (request, socket, head) => {
    socket.on("error", () => {}); // absorb EPIPE/ECONNRESET during WS handshake
  ...
  wss.on("connection", (ws) => {
    ...
    // Welcome before clients.add so publishAll cannot reach this socket first.
    void runPromise(
