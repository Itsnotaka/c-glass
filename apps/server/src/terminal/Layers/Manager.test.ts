...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "glass-terminal-" });
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.GLASS_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            GLASS_PROJECT_ROOT: "/repo",
            GLASS_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.GLASS_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.GLASS_WORKTREE_PATH, "/repo/worktree-a");
