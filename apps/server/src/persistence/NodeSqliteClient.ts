...
type SqliteBind = null | number | bigint | string | NodeJS.ArrayBufferView;
...
export const SqliteClient = ServiceMap.Service<Client.SqlClient>(
  "glass/persistence/NodeSqliteClient",
);
  ...
  Effect.gen(function* () {
    ...
    const makeConnection = Effect.gen(function* () {
      ...
      const runStatement = (
        ...
        Effect.withFiber<ReadonlyArray<object>, SqlError>((fiber) => {
          statement.setReadBigInts(Boolean(ServiceMap.get(fiber.services, Client.SafeIntegers)));
          try {
            if (hasRows(statement)) {
              return Effect.succeed(statement.all(...(params as SqliteBind[])));
            }
            const result = statement.run(...(params as SqliteBind[]));
            return Effect.succeed(raw ? (result as unknown as ReadonlyArray<object>) : []);
          } catch (cause) {
            return Effect.fail(
              new SqlError({
                reason: classifySqliteError(cause, {
            ...
            Effect.try({
              try: () => {
                if (hasRows(statement)) {
                  ...
                  return statement.all(...(params as SqliteBind[])) as unknown as ReadonlyArray<
                    ReadonlyArray<unknown>
                  >;
                }
                statement.run(...(params as SqliteBind[]));
                return [];
              },
              catch: (cause) =>
                new SqlError({
