...
function exploredSummary(tool: Extract<ChatRow, { kind: "tool" }>): string {
...
const ExploredToolItem = memo(function ExploredToolItem(props: {
  tool: Extract<ChatRow, { kind: "tool" }>;
}) {
...
const ExploredCard = memo(function ExploredCard(props: {
  row: Extract<ChatRow, { kind: "explored" }>;
  expanded: boolean;
}) {
  const [open, setOpen] = useSyncedExpand(props.expanded);
...
const GlassChatList = memo(function GlassChatList(props: { rows: ChatRow[]; expanded: boolean }) {
  return props.rows.map((row) => draw(row, props.expanded));
});

const GlassChatTranscript = memo(function GlassChatTranscript(props: {
  items: GlassSessionItem[];
  expanded: boolean;
  wide: number;
}) {
  const rows = useMemo(() => buildChatRows(props.items), [props.items]);
  return <GlassChatList rows={rows} expanded={props.expanded} />;
});

const GlassChatLive = memo(function GlassChatLive(props: {
  item: GlassSessionItem | null;
  expanded: boolean;
  wide: number;
}) {
  const rows = useMemo(() => (props.item ? buildChatRows([props.item]) : []), [props.item]);
  return <GlassChatList rows={rows} expanded={props.expanded} />;
});

/** Dev `/dev/icons`: real transcript chrome that uses `central-icons` in this file. */
export function GlassChatRowsIconStripDemo() {
  const running: Extract<ChatRow, { kind: "tool" }> = {
    id: "demo-run",
    kind: "tool",
    name: "read",
    args: '{"path":"x"}',
  ...
  const bash: Extract<ChatRow, { kind: "bash" }> = {
    id: "demo-bash",
    kind: "bash",
    command: "ls",
    output: "",
  ...
  const glob: Extract<ChatRow, { kind: "tool" }> = {
    id: "demo-glob",
    kind: "tool",
    name: "glob",
    args: '{"pattern":"**"}',
  ...
  const fileAtt: ChatUserAttachment = {
    kind: "file",
    name: "a.ts",
    path: "/a.ts",
    note: "",
  };
  const imgAtt: ChatUserAttachment = {
    kind: "image",
    name: "b.png",
    path: null,
    note: "",
