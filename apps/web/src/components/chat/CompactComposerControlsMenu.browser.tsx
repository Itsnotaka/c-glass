import { DEFAULT_MODEL_BY_PROVIDER, ModelSelection, ThreadId } from "@glass/contracts";
import "../../index.css";
...
async function mountMenu(props?: { modelSelection?: ModelSelection; prompt?: string }) {
  ...
  >["draftsByThreadId"];
  const model = props?.modelSelection?.model ?? DEFAULT_MODEL_BY_PROVIDER.pi;
...
describe("CompactComposerControlsMenu", () => {
  ...
  it("shows fast mode controls for Opus", async () => {
    await using _ = await mountMenu({
      modelSelection: { provider: "pi", model: "claude-opus-4-6" },
    });
