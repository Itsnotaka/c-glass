import { type ProviderKind, type ServerProvider } from "@glass/contracts";
...
import { DEFAULT_UNIFIED_SETTINGS } from "@glass/contracts/settings";
...
describe("ProviderModelPicker", () => {
  ...
  it("opens provider submenus with a visible gap from the parent menu", async () => {
    ...
    try {
      await page.getByRole("button").click();
      const providerTrigger = page.getByRole("menuitem", { name: "Pi" });
      ...
      ).find((element) => element.textContent?.includes("Pi"));
      if (!providerTriggerElement) {
        throw new Error("Expected the Pi provider trigger to be mounted.");
      }
  ...
  it("only shows codex spark when the server reports it for the account", async () => {
    const claudeModels = TEST_PROVIDERS[0]!.models.filter((m) => m.slug.startsWith("claude"));
    ...
    try {
      await page.getByRole("button").click();
      await page.getByRole("menuitem", { name: "Pi" }).hover();
  ...
  it("dispatches the canonical slug when a model is selected", async () => {
    ...
    try {
      ...
      expect(mounted.onProviderModelChange).toHaveBeenCalledWith("pi", "claude-sonnet-4-6");
    } finally {
      await mounted.cleanup();
    }
  });
  ...
  it("shows disabled providers as non-selectable entries", async () => {
    ...
    const piIndex = disabledProviders.findIndex((provider) => provider.provider === "pi");
    if (piIndex >= 0) {
      const piProvider = disabledProviders[piIndex]!;
      disabledProviders[piIndex] = {
        ...piProvider,
        enabled: false,
        status: "disabled",
      };
