...
const REASONING_EFFORT_VALUES = new Set<string>(CODEX_REASONING_EFFORT_OPTIONS);
...
function getRawEffort(modelOptions: PiModelOptions | null | undefined): string | null {
  const o = modelOptions;
  if (!o) return null;
  const fromReasoning = trimOrNull(o.reasoningEffort ?? null);
  if (fromReasoning) return fromReasoning;
  return trimOrNull(o.effort ?? null);
}

function getRawContextWindow(modelOptions: PiModelOptions | null | undefined): string | null {
  return trimOrNull(modelOptions?.contextWindow ?? null);
}

function buildNextOptions(
  modelOptions: ProviderOptions | null | undefined,
  patch: Partial<PiModelOptions>,
): ProviderOptions {
  return { ...(modelOptions as PiModelOptions | undefined), ...patch };
}
...
function getSelectedTraits(
  ...
  const rawEffort = getRawEffort(modelOptions as PiModelOptions | null | undefined);
  const effort = resolveEffort(caps, rawEffort) ?? null;

  const thinkingEnabled = caps.supportsThinkingToggle
    ? ((modelOptions as PiModelOptions | undefined)?.thinking ?? true)
    : null;
  ...
  const rawContextWindow = getRawContextWindow(modelOptions as PiModelOptions | null | undefined);
...
export const TraitsMenuContent = memo(function TraitsMenuContentImpl({
  ...
  const handleEffortChange = useCallback(
    (value: string) => {
      ...
      const effortKey = REASONING_EFFORT_VALUES.has(nextOption.value)
        ? "reasoningEffort"
        : "effort";
      updateModelOptions(
        buildNextOptions(modelOptions, {
          [effortKey]: nextOption.value,
        } as Partial<PiModelOptions>),
      );
    },
    [
      ultrathinkPromptControlled,
      ...
      {effort ? (
        ...
        <MenuGroup>
          ...
          <MenuRadioGroup
            ...
            onValueChange={(value) => {
              updateModelOptions(buildNextOptions(modelOptions, { thinking: value === "on" }));
            }}
          >
        ...
        <>
          ...
          <MenuGroup>
            ...
            <MenuRadioGroup
              ...
              onValueChange={(value) => {
                updateModelOptions(buildNextOptions(modelOptions, { fastMode: value === "on" }));
              }}
            >
              <MenuRadioItem value="off">off</MenuRadioItem>
        ...
        <>
          ...
          <MenuGroup>
            ...
            <MenuRadioGroup
              ...
              onValueChange={(value) => {
                updateModelOptions(buildNextOptions(modelOptions, { contextWindow: value }));
              }}
            >
