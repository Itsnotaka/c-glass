...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  {
    ...
    homePathKey: "piHomePath",
    homePlaceholder: "PI_HOME",
    homeDescription: "Optional custom Pi home and config directory.",
  },
] as const;
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Glass.",
    };
  }
...
export function GeneralSettingsPanel() {
  ...
  const piHomePath = settings.providers.pi.homePath;
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how Glass looks across the app."
          resetAction={
            theme !== "system" ? (
                    ...
                    {providerCard.homePathKey ? (
                      <div className="border-t border-border/60 px-4 py-3 sm:px-5">
                        <label
                          ...
                          className="block"
                        >
                          <span className="text-xs font-medium text-foreground">PI_HOME path</span>
                          <Input
