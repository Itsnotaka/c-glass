...
function getComposerInlineTokenTextLength(): 1 {
  return 1;
}
...
function getAbsoluteOffsetForInlineTokenPoint(
  ...
  return absoluteOffset + (pointOffset > 0 ? getComposerInlineTokenTextLength() : 0);
}
...
function findSelectionPointForInlineToken(
  ...
  if (remainingRef.value === getComposerInlineTokenTextLength()) {
    return {
      key: parent.getKey(),
      offset: index + 1,
      type: "element",
...
>(function ComposerPromptEditor(
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "glass-composer-editor",
      editable: true,
