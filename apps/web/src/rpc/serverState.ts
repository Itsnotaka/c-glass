...
const makeStateAtom = <A>(label: string, initialValue: A) =>
...
export const getServerConfig = (): ServerConfig | null => appAtomRegistry.get(serverConfigAtom);

export const getServerConfigUpdatedNotification = (): ServerConfigUpdatedNotification | null =>
  appAtomRegistry.get(serverConfigUpdatedAtom);
...
export const onWelcome = (listener: (payload: ServerLifecycleWelcomePayload) => void) =>
  subscribeLatest(welcomeAtom, listener);

export const onServerConfigUpdated = (
  listener: (payload: ServerConfigUpdatedPayload, source: ServerConfigUpdateSource) => void,
) =>
  subscribeLatest(serverConfigUpdatedAtom, (notification) => {
    listener(notification.payload, notification.source);
  });

export const onProvidersUpdated = (listener: (payload: ServerProviderUpdatedPayload) => void) =>
  subscribeLatest(providersUpdatedAtom, listener);
...
export const useServerConfig = (): ServerConfig | null => useAtomValue(serverConfigAtom);

export const useServerSettings = (): ServerSettings =>
  useAtomValue(serverConfigAtom, selectSettings);

export const useServerProviders = (): ReadonlyArray<ServerProvider> =>
  useAtomValue(serverConfigAtom, selectProviders);

export const useServerKeybindings = (): ServerConfig["keybindings"] =>
  useAtomValue(serverConfigAtom, selectKeybindings);

export const useServerAvailableEditors = (): ReadonlyArray<EditorId> =>
  useAtomValue(serverConfigAtom, selectAvailableEditors);

export const useServerKeybindingsConfigPath = (): string | null =>
  useAtomValue(serverConfigAtom, selectKeybindingsConfigPath);

export const useServerObservability = (): ServerConfig["observability"] | null =>
  useAtomValue(serverConfigAtom, selectObservability);
