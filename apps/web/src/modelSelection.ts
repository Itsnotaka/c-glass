...
import { normalizeModelSlug, resolveSelectableModel } from "@glass/shared/model";
...
const PROVIDER_CUSTOM_MODEL_CONFIG: Record<ProviderKind, ProviderCustomModelConfig> = {
  pi: {
    provider: "pi",
    title: "Pi",
    description: "Save additional model slugs for the picker and `/model` command.",
...
export function resolveAppModelSelectionState(
  ...
  const { modelOptionsForDispatch } = getComposerProviderState({
    ...
    prompt: "",
    modelOptions: {
      pi: provider === selection.provider ? selection.options : undefined,
    },
  });
