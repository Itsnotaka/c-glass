...
import { UnifiedSettings } from "@glass/contracts/settings";

export const COMPOSER_DRAFT_STORAGE_KEY = "glass:composer-drafts:v1";
...
function normalizeProviderKind(value: unknown): ProviderKind | null {
  ...
  if (value === "codex" || value === "claudeAgent") return "pi";
  return null;
}
...
function normalizeProviderModelOptions(
  ...
  const reasoningSource = piCandidate ?? codexCandidate;
  let codexReasoningEffort: CodexReasoningEffort | undefined;
  if (
    reasoningSource?.reasoningEffort === "low" ||
    reasoningSource?.reasoningEffort === "medium" ||
    reasoningSource?.reasoningEffort === "high" ||
    reasoningSource?.reasoningEffort === "xhigh"
  ) {
    codexReasoningEffort = reasoningSource.reasoningEffort as CodexReasoningEffort;
  } else if (
    legacy?.effort === "low" ||
    ...
    codexReasoningEffort = legacy.effort;
  }

  let fastMode: boolean | undefined;
  if (piCandidate?.fastMode === true || codexCandidate?.fastMode === true) {
    fastMode = true;
  } else if (piCandidate?.fastMode === false || codexCandidate?.fastMode === false) {
    fastMode = false;
  } else if (
  ...
  if (piCandidate?.thinking === true || claudeCandidate?.thinking === true) {
    thinking = true;
  } else if (piCandidate?.thinking === false || claudeCandidate?.thinking === false) {
    thinking = false;
  }

  const effortSource = piCandidate ?? claudeCandidate;
  let effort: ClaudeCodeEffort | undefined;
  if (
    effortSource?.effort === "low" ||
    effortSource?.effort === "medium" ||
    effortSource?.effort === "high" ||
    effortSource?.effort === "max" ||
    effortSource?.effort === "ultrathink"
  ) {
    effort = effortSource.effort as ClaudeCodeEffort;
  }

  let contextWindow: string | undefined;
  const cw =
    typeof piCandidate?.contextWindow === "string" && piCandidate.contextWindow.length > 0
      ? piCandidate.contextWindow
  ...
  return Object.keys(pi).length > 0 ? { pi } : null;
}
...
function normalizeModelSelection(
  ...
  const modelOptions = normalizeProviderModelOptions(
    candidate?.options ? { pi: candidate.options } : legacy?.modelOptions,
    legacy?.legacyCodex,
  );
...
function legacyReplaceProviderModelOptions(
  ...
  const otherProviderModelOptions = { ...(currentModelOptions ?? {}) };
  delete otherProviderModelOptions[provider];
  ...
  for (const [threadId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
    ...
    if (
      ...
      const normalizedModelOptions =
        normalizeProviderModelOptions(legacyDraftCandidate.modelOptions, legacyDraftCandidate) ??
        null;
      ...
      clearDraftThread: (threadId) => {
        ...
        set((state) => {
          ...
          const restDraftThreadsByThreadId = Object.fromEntries(
            Object.entries(state.draftThreadsByThreadId).filter(([id]) => id !== threadId),
          ) as Record<ThreadId, DraftThreadState>;
          const restDraftsByThreadId = Object.fromEntries(
            Object.entries(state.draftsByThreadId).filter(([id]) => id !== threadId),
