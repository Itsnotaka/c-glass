...
describe("composerDraftStore modelSelection", () => {
  ...
  it("stores a model selection in the draft", () => {
    ...
    store.setModelSelection(
      threadId,
      modelSelection("pi", "gpt-5.3-codex", {
  ...
  it("keeps default-only model selections on the draft", () => {
    const store = useComposerDraftStore.getState();
    store.setModelSelection(threadId, modelSelection("pi", "gpt-5.4"));

    expect(
      useComposerDraftStore.getState().draftsByThreadId[threadId]?.modelSelectionByProvider.pi,
    ).toEqual(modelSelection("pi", "gpt-5.4"));
  });
  ...
  it("replaces only the targeted provider options on the current model selection", () => {
    ...
    store.setModelSelection(
      threadId,
      modelSelection("pi", "claude-opus-4-6", {
        effort: "max",
        fastMode: true,
      }),
    );
  ...
  it("keeps explicit off/default codex overrides on the selection", () => {
    ...
    store.setModelSelection(threadId, modelSelection("pi", "gpt-5.4", { fastMode: true }));
  ...
  it("updates only the draft when sticky persistence is omitted", () => {
    ...
    store.setStickyModelSelection(modelSelection("pi", "claude-opus-4-6", { effort: "max" }));
    store.setModelSelection(threadId, modelSelection("pi", "claude-opus-4-6", { effort: "max" }));
    ...
    expect(useComposerDraftStore.getState().stickyModelSelectionByProvider.pi).toEqual(
      modelSelection("pi", "claude-opus-4-6", { effort: "max" }),
    );
  });

  it("replaces pi options when setModelOptions is called again for pi", () => {
    const store = useComposerDraftStore.getState();

    store.setModelOptions(
      threadId,
      providerModelOptions({
        pi: { fastMode: true, effort: "max" },
      }),
    );

    store.setModelOptions(threadId, providerModelOptions({ pi: { reasoningEffort: "xhigh" } }));

    const draft = useComposerDraftStore.getState().draftsByThreadId[threadId];
    expect(draft?.modelSelectionByProvider.pi?.options).toEqual({ reasoningEffort: "xhigh" });
  });

  it("preserves pi model options when switching the active model", () => {
    const store = useComposerDraftStore.getState();
    ...
    store.setModelSelection(threadId, modelSelection("pi", "claude-opus-4-6"));

    const draft = useComposerDraftStore.getState().draftsByThreadId[threadId];
    expect(draft?.modelSelectionByProvider.pi).toEqual(
      modelSelection("pi", "claude-opus-4-6", { fastMode: true, effort: "max" }),
    );
...
describe("composerDraftStore setModelSelection", () => {
  ...
  it("keeps explicit model overrides instead of coercing to null", () => {
    ...
    store.setModelSelection(threadId, modelSelection("pi", "gpt-5.3-codex"));

    expect(
      useComposerDraftStore.getState().draftsByThreadId[threadId]?.modelSelectionByProvider.pi,
    ).toEqual(modelSelection("pi", "gpt-5.3-codex"));
  });
});
...
describe("composerDraftStore sticky composer settings", () => {
  ...
  it("normalizes empty sticky model options by dropping selection options", () => {
    ...
    store.setStickyModelSelection(modelSelection("pi", "gpt-5.4"));
  ...
  it("applies sticky activeProvider to new drafts", () => {
    ...
    store.setStickyModelSelection(modelSelection("pi", "claude-opus-4-6"));
    store.applyStickyState(threadId);

    expect(useComposerDraftStore.getState().draftsByThreadId[threadId]).toMatchObject({
      modelSelectionByProvider: {
        pi: modelSelection("pi", "claude-opus-4-6"),
      },
      activeProvider: "pi",
    });
  });
...
describe("composerDraftStore provider-scoped option updates", () => {
  ...
  it("applies provider option updates to the current pi selection", () => {
    const store = useComposerDraftStore.getState();
    store.setModelSelection(
      threadId,
      modelSelection("pi", "gpt-5.3-codex", {
    ...
    store.setProviderModelOptions(threadId, "pi", { effort: "max" });
    const draft = useComposerDraftStore.getState().draftsByThreadId[threadId];
    expect(draft?.modelSelectionByProvider.pi).toEqual(
      modelSelection("pi", "gpt-5.3-codex", { effort: "max" }),
    );
    expect(draft?.activeProvider).toBe("pi");
  });
});
