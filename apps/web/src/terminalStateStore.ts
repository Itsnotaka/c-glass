...
import type { ThreadId } from "@glass/contracts";
...
const TERMINAL_STATE_STORAGE_KEY = "glass:terminal-state:v1";
...
function updateTerminalStateByThreadId(
  ...
  if (isDefaultThreadTerminalState(next)) {
    ...
    return Object.fromEntries(
      Object.entries(terminalStateByThreadId).filter(([id]) => id !== threadId),
