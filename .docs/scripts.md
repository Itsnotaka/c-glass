...
- Dev commands use `GLASS_HOME` (default `~/.glass`); server state for `bun run dev` lives under `<GLASS_HOME>/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.glass-2`
...
- Desktop production windows load the bundled UI from `glass://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the Glass backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `GLASS_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `GLASS_DEV_INSTANCE`)
- Example: `GLASS_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `GLASS_PORT_OFFSET` to a numeric offset.
